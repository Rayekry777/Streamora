...
const DOCKER_PATH = /(?:^|\/)(?:Dockerfile|docker-compose[^/]*\.ya?ml)$|^platform\/(?:compose|docker)\/|^scripts\/ci\/(?:deploy|run-(?:deployed|functional-docker|stage-acceptance)|collect-deploy|stage-manifest)/;
...
export function classifyChanges(inputFiles, options = {}) {
  ...
  for (const file of files) {
    ...
    docker ||= DOCKER_PATH.test(file) || MIGRATION_PATH.test(file) || REAL_ADAPTER_PATH.test(file);

    const service = backendService(file);
    if (service) {
      backendModules.add(`services/${service}`);
      if (DOCKER_PATH.test(file) || MIGRATION_PATH.test(file) || REAL_ADAPTER_PATH.test(file)) {
        dockerServices.add(service);
      }
    }
    const web = frontendService(file);
    if (web && DOCKER_PATH.test(file)) {
      dockerServices.add(web);
    }
  }
  ...
  if (backendModules.size > 1 || files.some((file) => file.startsWith('packages/proto/'))) {
    docker = true;
  }
  if (backendModules.size > 1) {
    for (const module of backendModules) {
      dockerServices.add(module.replace(/^services\//, ''));
    }
  }
...
export function githubOutputs(classification) {
  return {
    ...
    docker: String(classification.docker),
    backend_modules: classification.backendModules.join(','),
    docker_services: classification.dockerServices.join(',')
  };
}

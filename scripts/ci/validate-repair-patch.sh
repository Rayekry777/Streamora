...
while IFS= read -r file; do
  [[ -n "$file" ]] || continue
  case "$file" in
    .env|.env.*|*/.env|*/.env.*|*application-secret*.yml|*application-secret*.yaml|*.pem|*.key|*.p12|*.pfx|*.jks|*.keystore|.github/*|*/db/migration/*|pnpm-lock.yaml|package-lock.json|yarn.lock|npm-shrinkwrap.json|composer.lock|Gemfile.lock)
